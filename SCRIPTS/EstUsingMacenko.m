...
I = reshape(double(I), [], 3); %into RGB matrix form

% calculate optical density
OD = -log((I+1)/Io);

% remove transparent pixels (only include values where OD>beta)
ODhat = OD(~any(OD < beta, 2), :);

% any(A,2) works on elements in the rows of A and returns a column vector of 1s if a nonzero value exists
% ~A returns an array containing 1(true) where A is zero and 0(false) where A is nonzero.
% If X and V are vectors, then X(V) is [X(V(1)), X(V(2)), ..., X(V(n))]
% 
